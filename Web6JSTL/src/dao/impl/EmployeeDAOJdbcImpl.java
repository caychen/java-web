...
public class EmployeeDAOJdbcImpl implements EmployeeDAO {
	...
	public void deleteEmp(int id) throws Exception {
		...
		try {
			...
			PreparedStatement state = conn
					.prepareStatement("delete from t_emp where id=?");
			state.setInt(1, id);
	 ...
	 */
	@Override
	public List<Employee> findAllByPage(int pages, int perPage)
			throws Exception {
		// TODO Auto-generated method stub
		List<Employee> employees = new ArrayList<Employee>();
		...
		try {
			...
			PreparedStatement state = conn.prepareStatement("select * from t_emp limit ?,?");
			state.setInt(1, (pages - 1) * perPage);
			state.setInt(2, perPage);
			ResultSet rs = state.executeQuery();
	...
	public int totalPages(int perPage) throws Exception {
		...
		try {
			...
			if(rs.next()){
				...
				totalPages = (totalRows % perPage == 0 ? (totalRows / perPage) : (totalRows / perPage + 1));
			}
